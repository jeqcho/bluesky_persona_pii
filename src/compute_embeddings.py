...
def compute_cluster_embedding(cluster_path: Path, manager: EmbeddingManager):
    ...
    with open(cluster_path, "r") as f:
        ...
        for line in tqdm(lines, desc=f"Loading {cluster_path.name}", leave=True):
            ...
            last_item = data["thread"][-1]
            assert isinstance(last_item, dict)

            # Skip if last action is not text-based.
            if "text" not in last_item:
                continue
            text = last_item["text"]
